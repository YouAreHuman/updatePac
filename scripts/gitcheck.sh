...
# 假设你要覆盖根目录下的 "${checkFile}.yaml"
cp cfw/"${checkFile}.yaml" ./"${checkFile}.yaml"
# 方法：检查 git status 是否有输出。
# git status --porcelain 会列出所有变更（包括新文件 M, A, D 等）。
# 如果没有任何输出，说明真的没变化。
# 如果有输出（例如 ?? "${checkFile}.yaml" 表示新文件），说明有变化。
# 初始化标记变量
HAS_CHANGES=false
...
if ! git diff --quiet HEAD -- "${checkFile}.yaml"; then
  echo "🔄 检测到 [内容修改]: "${checkFile}.yaml" 内容已更新"
  HAS_CHANGES=true
fi

# --- 第二步：检查是否为新文件 (未跟踪) ---
# 如果文件是新的 (Untracked)，git diff 上面那步会跳过，但这步能抓到
# git status --porcelain 输出非空表示有状态变化 (如 ?? "${checkFile}.yaml")
CHANGES_STATUS=$(git status --porcelain -- "${checkFile}.yaml")
if [ -n "$CHANGES_STATUS" ]; then
  # 避免重复打印，如果是新文件，上面 diff 没抓到，这里才打印
  if [ "$HAS_CHANGES" = false ]; then
    echo "🔄 检测到 [新文件]: "${checkFile}.yaml" 是新创建的"
  else
